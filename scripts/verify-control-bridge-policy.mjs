...
assert.match(investigationConfig, /apiKey:\s*["']\$\{NVIDIA_API_KEY\}["']/, 'standalone NVIDIA provider must resolve its server-side environment variable locally');
assert.match(investigationConfig, /apiKey:\s*["']\$\{OPENROUTER_API_KEY\}["']/, 'standalone OpenRouter provider must resolve its server-side environment variable locally');
assert.doesNotMatch(investigationConfig, /apiKey:\s*\{\s*source:\s*["']env["'][\s\S]{0,120}id:\s*["'](?:NVIDIA_API_KEY|OPENROUTER_API_KEY)["']/, 'standalone case config must not depend on Gateway secrets.resolve for model credentials');
