...
const zenInvestigationConfig = read('infra/openclaw/integritas-investigation-zen.json5');
...
assert.match(browserHelper, /openclaw health/, 'browser readiness helper must wait for Gateway health');
assert.match(browserHelper, /browser --browser-profile openclaw start/, 'browser readiness helper must start only the named operator profile');
assert.match(browserHelper, /browser --browser-profile openclaw doctor/, 'browser readiness helper must verify browser readiness');
assert.match(browserService, /^NoNewPrivileges=true$/m, 'managed browser launcher must retain NoNewPrivileges');
assert.ok(!browserService.match(/^ExecStart=.*\b(sh -c|bash -c|sudo)\b/m), 'managed browser service must not launch through a shell or sudo');
assert.ok(!browserHelper.includes('eval '), 'browser readiness helper must not invoke eval');
assert.ok(!browserHelper.includes('sudo '), 'browser readiness helper must run entirely as the openclaw service account');
...
assert.ok(investigationRunner.includes("const ZEN_ENABLE_MARKER = '/etc/openclaw/zen-enabled'"), 'Zen activation must require an explicit enable marker');
...
assert.ok(!investigationConfig.includes('OPENCODE_ZEN_API_KEY'), 'active investigation config must not resolve the optional Zen secret');
assert.match(zenInvestigationConfig, /"integritas-opencode-zen"/, 'staged Zen config must define the OpenCode Zen chat provider');
assert.match(zenInvestigationConfig, /"integritas-opencode-zen-responses"/, 'staged Zen config must define the OpenCode Zen responses provider');
assert.match(zenInvestigationConfig, /id:\s*["']OPENCODE_ZEN_API_KEY["']/, 'staged Zen config must use the optional Zen SecretRef');
assert.match(zenActivator, /OPENCODE_ZEN_API_KEY/, 'Zen activation helper must require the server-side key');
assert.match(zenActivator, /zen-enabled/, 'Zen activation helper must own the enable marker');
assert.match(zenActivator, /config validate/, 'Zen activation helper must validate the staged config before enabling');
