...
create index if not exists integritas_case_job_outputs_case_id_idx
  on public.integritas_case_job_outputs(case_id);

create index if not exists integritas_control_audit_command_id_idx
  on public.integritas_control_audit(command_id);

create index if not exists integritas_finding_source_links_case_id_idx
  on public.integritas_finding_source_links(case_id);

create index if not exists integritas_finding_source_links_finding_id_idx
  on public.integritas_finding_source_links(finding_id);

create index if not exists integritas_finding_source_links_source_id_idx
  on public.integritas_finding_source_links(source_id);
