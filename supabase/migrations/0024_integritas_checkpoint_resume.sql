...
-- Continue/retry is a checkpoint resume. It must never reset a Deep/Maximum
-- investigation to queued/0 simply because a prior bounded run ended incomplete.
-- The Oracle workspace validates each retained phase artifact against the current
-- manifest before reuse, so this database checkpoint is the durable lower bound
-- for visible progress while only missing/invalid phases are recomputed.
