...
-- Provenance and stable job-local keys for structured investigation materialization.
alter table public.integritas_entities
  add column if not exists case_job_id uuid null references public.integritas_case_jobs(id) on delete set null,
  add column if not exists case_revision integer null,
  add column if not exists entity_key text null;
alter table public.integritas_relationships
  add column if not exists case_job_id uuid null references public.integritas_case_jobs(id) on delete set null,
  add column if not exists case_revision integer null,
  add column if not exists relationship_key text null,
...
alter table public.integritas_checks
  add column if not exists case_job_id uuid null references public.integritas_case_jobs(id) on delete set null,
  add column if not exists case_revision integer null,
  add column if not exists check_key text null;
alter table public.integritas_reports
  add column if not exists case_job_id uuid null references public.integritas_case_jobs(id) on delete set null,
  add column if not exists report_key text null,
  add column if not exists bundle_sha256 text null,
  add column if not exists report_sha256 text null,
  add column if not exists updated_at timestamptz not null default now();

alter table public.integritas_entities
  add constraint integritas_entities_case_revision_check check (case_revision is null or case_revision >= 0),
  add constraint integritas_entities_entity_key_check check (entity_key is null or entity_key ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$');
alter table public.integritas_relationships
  add constraint integritas_relationships_case_revision_check check (case_revision is null or case_revision >= 0),
  add constraint integritas_relationships_relationship_key_check check (relationship_key is null or relationship_key ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$'),
  add constraint integritas_relationships_confidence_check check (confidence is null or confidence between 0 and 100);
alter table public.integritas_findings
  add constraint integritas_findings_finding_key_check check (finding_key is null or finding_key ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$');
alter table public.integritas_sources
  add constraint integritas_sources_case_revision_check check (case_revision is null or case_revision >= 0),
  add constraint integritas_sources_source_key_check check (source_key is null or source_key ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$');
alter table public.integritas_checks
  add constraint integritas_checks_case_revision_check check (case_revision is null or case_revision >= 0),
  add constraint integritas_checks_check_key_check check (check_key is null or check_key ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$');
alter table public.integritas_reports
  add constraint integritas_reports_report_key_check check (report_key is null or report_key ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$'),
  add constraint integritas_reports_bundle_sha256_check check (bundle_sha256 is null or bundle_sha256 ~ '^[0-9a-f]{64}$'),
  add constraint integritas_reports_report_sha256_check check (report_sha256 is null or report_sha256 ~ '^[0-9a-f]{64}$');

create unique index integritas_entities_job_key on public.integritas_entities(case_job_id, entity_key)
  where case_job_id is not null and entity_key is not null;
create unique index integritas_relationships_job_key on public.integritas_relationships(case_job_id, relationship_key)
  where case_job_id is not null and relationship_key is not null;
create unique index integritas_findings_job_key on public.integritas_findings(case_job_id, finding_key)
  where case_job_id is not null and finding_key is not null;
create unique index integritas_sources_job_key on public.integritas_sources(case_job_id, source_key)
  where case_job_id is not null and source_key is not null;
create unique index integritas_checks_job_key on public.integritas_checks(case_job_id, check_key)
  where case_job_id is not null and check_key is not null;
create unique index integritas_reports_job_key on public.integritas_reports(case_job_id, report_key)
  where case_job_id is not null and report_key is not null;

create table public.integritas_finding_source_links (
  ...
  finding_id uuid not null references public.integritas_findings(id) on delete cascade,
  source_id uuid not null references public.integritas_sources(id) on delete cascade,
  created_at timestamptz not null default now(),
  unique (case_job_id, finding_id, source_id)
);
alter table public.integritas_finding_source_links enable row level security;
revoke all on table public.integritas_finding_source_links from anon, authenticated;
grant select on table public.integritas_finding_source_links to authenticated;
grant all on table public.integritas_finding_source_links to service_role;
create policy "read authorized case rows"
  on public.integritas_finding_source_links for select to authenticated
  using (integritas_private.integritas_can_access_case(case_id));

-- Canonical report output is Markdown. HTML remains legacy/derived only.
...
declare
  ...
  v_prefix text;
begin
  if p_output_type not in ('bundle','report_markdown','report_html','evidence','execution_log') then
    raise exception 'invalid investigation output type';
  end if;
  if p_content_type not in ('application/json','text/html','application/octet-stream','text/plain','text/markdown') then
    raise exception 'invalid investigation content type';
  end if;
  if p_sha256 is null or p_sha256 !~ '^[0-9a-f]{64}$' then raise exception 'invalid investigation output digest'; end if;
  if p_size_bytes is null or p_size_bytes < 0 or p_size_bytes > 5242880 then raise exception 'invalid investigation output size'; end if;
  ...
  for update of j;
  if not found then raise exception 'investigation output access denied'; end if;
  ...
  insert into public.integritas_case_job_outputs(
    case_id, case_job_id, case_revision, output_type, content_type, storage_path, sha256, size_bytes, safe_metadata
  ) values (
    v_job.case_id, v_job.id, v_job.case_revision, p_output_type, p_content_type, p_storage_path, p_sha256, p_size_bytes, p_safe_metadata
  )
  on conflict (case_job_id, output_type, sha256) do update set
    ...
    size_bytes = excluded.size_bytes, safe_metadata = excluded.safe_metadata
  returning * into v_output;
  return v_output;
end;
$$;

create or replace function integritas_private.integritas_commit_investigation_bundle(
  p_command_id uuid,
  p_worker_id text,
  p_case_job_id uuid,
...
begin
  ...
  if found and (v_existing_report.bundle_sha256 <> p_bundle_sha256 or v_existing_report.report_sha256 <> p_report_sha256) then
    raise exception 'investigation bundle digest conflict';
  end if;
...
revoke all on function integritas_private.integritas_commit_investigation_bundle(uuid,text,uuid,integer,text,text,jsonb) from public, anon, authenticated;
grant execute on function integritas_private.integritas_commit_investigation_bundle(uuid,text,uuid,integer,text,text,jsonb) to service_role;

create or replace function public.integritas_commit_investigation_bundle(
  p_command_id uuid, p_worker_id text, p_case_job_id uuid, p_case_revision integer,
  p_bundle_sha256 text, p_report_sha256 text, p_bundle jsonb
) returns jsonb
language sql security definer set search_path = ''
as $$
  select integritas_private.integritas_commit_investigation_bundle(
    p_command_id,p_worker_id,p_case_job_id,p_case_revision,p_bundle_sha256,p_report_sha256,p_bundle
  );
$$;
revoke all on function public.integritas_commit_investigation_bundle(uuid,text,uuid,integer,text,text,jsonb) from public, anon, authenticated;
grant execute on function public.integritas_commit_investigation_bundle(uuid,text,uuid,integer,text,text,jsonb) to service_role;
