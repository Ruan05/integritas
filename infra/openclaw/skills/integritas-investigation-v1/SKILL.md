...
Use a tool ladder rather than repeating the same search. Use `web_search` first for broad discovery through the configured healthy search provider, then open the underlying source. Use `web_fetch` for stable pages; the production profile routes it through the official Firecrawl fetch provider so difficult extraction can fall back without changing case logic. Use browser automation for dynamic/JavaScript portals, interactive registries, search forms, issuer-verification pages, archive navigation, login-free public forms, and evidence pages that `web_fetch` cannot reliably inspect. Use `pdf` and `view_image` for visual/document review. If a provider-specific tool is exposed and credentialed for the run, use Exa for semantic/entity discovery and Firecrawl search/scrape for difficult multi-page or structured extraction, but never make a material conclusion depend on one provider. Do not use search-result snippets as final evidence when the underlying source can be opened. Record tool-limited or access-blocked sources honestly and move to the next independent route rather than retrying the same failed call.
...
6. **Document Forensics & Internal Consistency** — metadata, signatures, edits, chronology, execution blocks, template/boilerplate signals, exact embedded-image reuse from trusted forensics, and document-to-document conflicts. For DocuSign/Adobe/other platform signatures, distinguish cryptographic revision integrity from signer identity, authority and truth of content; request issuer/envelope/audit evidence when material. Exact image/template reuse is a provenance signal, not proof of common ownership or fraud. State the limits of what file properties prove.
